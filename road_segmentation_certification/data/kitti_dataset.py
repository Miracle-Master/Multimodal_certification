...
#rewrite this function for the baseline (radomized ablation). use test_mode to control whether the ablation percentage is fixed 
...
class kittidataset(BaseDataset):
    ...
    def initialize(self, opt):
        ...
        if opt.phase == "train":
            self.image_list = list_files(os.path.join(self.root, 'training', 'image_2'))
            print(self.image_list)
            #sorted(glob.glob(os.path.join(self.root, 'training', 'image_2', '*.png')))
        elif opt.phase == "val":
            self.image_list = list_files(os.path.join(self.root, 'training', 'image_2'))
            print(self.image_list)
            #sorted(glob.glob(os.path.join(self.root, 'validation', 'image_2', '*.png')))
        else:
            self.image_list = list_files(os.path.join(self.root, 'training', 'image_2'))#sorted(glob.glob(os.path.join(self.root, 'testing', 'image_2', '*.png')))
    ...
    def __getitem__(self, index):
        ...
        #print(os.path.join(useDir, 'depth_u16', name), cv2.IMREAD_ANYDEPTH)
        ...
        #print(depth_image)
        #Image.fromarray(depth_image/1000).show()
        if self.phase == "train":
        ...
        if self.phase == "test":
            if self.opt.certification_method == "MMCert":
                ...
                #shapes are: (375, 1242, 3) (375, 1242)
                ...
                #Image.fromarray(depth_image.astype(np.float32)/1000).show()
            #print(rgb_image.shape, label.shape, depth_image.shape)
